			...
			<table class="search_table" align="center" width="98%"> 
				<tr> 
					<td align="left"> 
						...
						<input name="cshareparamcode" type="text" id="cshareparamcode" value="<%=StringUtil.getNotEmptyStr(domainInstance.getCshareparamcode())%>" size="20" > 
						&nbsp;  
			...
			<table class="table table-bordered table-striped" align="center" width="98%"> 
				<thead> 
					<tr> 
						...
						<th><%=domainInstance.getPropertyCnName("cshareparamname") %></th> 
						<th><%=domainInstance.getPropertyCnName("cworkitemcode") %></th> 
						<th><%=domainInstance.getPropertyCnName("cbasemonth") %></th> 
						<th><%=domainInstance.getPropertyCnName("csystem") %></th> 
						<th><%=domainInstance.getPropertyCnName("bend") %></th> 
						<th>操作</th> 
					</tr> 
				</thead> 
				<% 
					for (int i = 0; i < list.size(); i++) 
						{ 
							EfCostShareparamObj o = (EfCostShareparamObj) list.get(i); 
				%> 
				<tr> 
					<td> 
					...
					<td><%=StringUtil.getNotEmptyStr(o.getCshareparamcode())%></td> 
					<td><%=StringUtil.getNotEmptyStr(o.getCshareparamname())%></td> 
					<td><%=new DictionaryService().getDictValueByDictKey("工作项目字典",o.getCworkitemcode())%></td>  
					<td><%=new DictionaryService().getDictValueByDictKey("是否字典",o.getCbasemonth())%></td>  
					<td><%=new DictionaryService().getDictValueByDictKey("是否字典",o.getCsystem())%></td>  
					<td><%=new DictionaryService().getDictValueByDictKey("是否字典",o.getBend())%></td>  
