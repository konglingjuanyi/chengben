	...
	<body> 
		...
		<table width="600" align="center" class="detail_table detail_table-bordered detail_table-striped"> 
			...
			<tr> 
				...
				<td><%=StringUtil.getNotEmptyStr(domainInstance.getAcc_code())%></td> 
			</tr> 
			<tr> 
				<td> 
					<%=domainInstance.getPropertyCnName("acc_name") %>: 
				</td> 
				<td><%=StringUtil.getNotEmptyStr(domainInstance.getAcc_name())%></td> 
			</tr> 
			<tr> 
				<td> 
					<%=domainInstance.getPropertyCnName("debit_or_credit") %>: 
				</td> 
				<td><%=new DictionaryService().getDictValueByDictKey("余额方向字典",domainInstance.getDebit_or_credit())%></td> 
			</tr> 
			<tr> 
				<td> 
					<%=domainInstance.getPropertyCnName("opposite_acc_code") %>: 
				</td> 
				<td><%=StringUtil.getNotEmptyStr(domainInstance.getOpposite_acc_code())%></td> 
			</tr> 
		</table> 
