...
<% 
	...
	// 对象实例 
	SourceDeptMapObj domainInstance = new SourceDeptMapObj(); 
	// 该功能基本路径 
	...
	{ 
		domainInstance = (SourceDeptMapObj) domainInstanceObj; 
	} 
...
<html> 
	<head> 
		...
		if(!checkNull("source_dept_name","<%=domainInstance.getPropertyCnName("source_dept_name")%>")) return false; 
		if(!checkNull("dest_dept_code","<%=domainInstance.getPropertyCnName("dest_dept_code")%>")) return false; 
	...
	<body> 
		<form name="addOrModifyForm" id="addOrModifyForm" action="<%=contextPath%>/<%=basePath%>/Servlet?method=addOrModify4this" method="post"> 
			...
			<table width="700" align="center" class="detail_table detail_table-bordered detail_table-striped"> 
				...
				<tr> 
					<td> 
						<%=domainInstance.getPropertyCnName("dest_dept_code") %>: 
					</td> 
					<td> 
						<input name="dest_dept_code" type="text" id="dest_dept_code" value="<%=StringUtil.getNotEmptyStr(domainInstance.getDest_dept_code(),"")%>" size="20" onfocus="openDictTable('dest')" readonly > 
						<input name="dest_dept_name" type="text" id="dest_dept_name" value="<%=StringUtil.isEmpty(domainInstance.getDest_dept_code())?"":(new DictionaryService().getDictValueByDictKey("医院科室字典",domainInstance.getDest_dept_code()))%>" size="20"  onfocus="openDictTable('dest')" readOnly> 
						<font color="red">*</font> 
					</td> 
				</tr> 
			</table> 
...
<table width="600" id="dest_dict_table" style="display:none; background-color:#ff9900; width:600px; margin-left: auto; margin-right: auto; padding:4px;" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<table id="dest_dict_table_list" width="100%" class="table table-bordered table-striped" align="center">
			...
			<%
				List<DictItem> items = new DictionaryService().getDictItemsByDictName("医院科室字典", true);
...
<script>
	...
	{		
		...
		if(check_radio)
		{
			$('#dest_dept_code').val($(check_radio).val());
			$('#dest_dept_name').val($(check_radio).attr('label'));
		}
